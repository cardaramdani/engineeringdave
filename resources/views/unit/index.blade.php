...
<!-- <form action="/kwhmeterunit/filter" method="post" class="form-inline col-5">
  @csrf
    <input type="date" 
            name="startdate" 
...
<h1 class="my-3">sace</h1>
                                                  <h1 align="center" class="font-weight-bold ">list Unit</h1>
@if (count($errors) > 0)
...
<tr>
  ...
  <td align="center" class="align-middle">{{$row->unit}}</td>
  <td align="center" class="align-middle">{{$row->daya}}</td>
  <td align="center" class="align-middle">{{$row->StandAkhir}}</td>
  <td align="center" class="align-middle">
    
  <form action="/unit/edit/{{$Unit->Unit}}" method="get" class="d-inline">
      <!-- @method('update') -->
      @csrf
    <button type="submit" class="btn btn-outline-success">detail</button> 
  </form>
  <form action="/Unit/delete/{{$Unit->id}}" method="post" class="d-inline" onclick=" return confirm ('Yakin Mau Di Delete?');">
      @method('delete')
      @csrf
